...
public class Charges extends JFrame implements ActionListener {
    ...
    public void Charges(String user) {
        ...
        // Query to retrieve the Guest data from the database and populate the Vector
        // object
        try {
            ...
            tbookingIdModel.addElement(new Item("-1", "--Select a Booking Number--"));
        ...
        // override toString() method of the Item class to populate the Combobox with
        // the data from the Vector
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == submit) {
            try {
                String s1 = "";
                String s2 = tbookingId.getSelectedItem().toString().substring(0,
                        tbookingId.getSelectedItem().toString().indexOf(' '));
                String s3 = thotelStaffId.getSelectedItem().toString().substring(0,
                        thotelStaffId.getSelectedItem().toString().indexOf(' '));// selected Booking Id
                String s4 = tamountCharged.getText();
                // entered Amount Charged
                String s5 = tchargeType.getSelectedItem().toString();
                ...
                if (!s2.equals("") && !s3.equals("") && !s4.equals("") && !s5.equals("")) {
                    ...
                    if (success > 0) {
                        query = "SELECT MAX(chargeId) FROM CHARGES WHERE bookingId = ? AND hotelStaffId = ? AND amountCharged = ? AND chargeType = ?";
                        da.ps = da.con.prepareStatement(query);
                        da.ps.setString(1, s2); // selected Booking ID
                        da.ps.setString(2, s3); // selected Staff ID
                        da.ps.setString(3, s4); // selected Charge Type
