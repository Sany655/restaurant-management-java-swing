...
/**
 * CreateBooking.java 05/04/22
 * pdf - 133 page same thing
...
public class CreateBooking extends JFrame implements ActionListener {
   ...
   public void createBooking(String user) {
      ...
      // Create Booking Sub Panel 2: The Date Booked textbox on the form MaskFormatter
      // dateBookedMask = null;
      try {
      ...
      // Query to retrieve the Room data from the database and populate the Vector
      // object
      try {
   ...
   public void actionPerformed(ActionEvent e) {
      if (e.getSource() == submit) {
         try {
            String s1 = "";
            String s2 = tguestId.getSelectedItem().toString().substring(0,
                  tguestId.getSelectedItem().toString().indexOf(' ')); // entered guest id
            String s3 = tbillToGuestId.getSelectedItem().toString().substring(0,
                  tbillToGuestId.getSelectedItem().toString().indexOf(' '));// entered booking billToGuestId;
            ...
            if (!s2.equals("") && !s3.equals("") && !s4.equals("") && !s5.equals("") && !s6.equals("")) {
               String query = "INSERT INTO BOOKING VALUES (null, ?, 1, 1, ?, ?, ?, 'Pending', ?, 0, ? )";
