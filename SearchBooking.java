...
import java.sql.*;
public class SearchGuestBooking extends JFrame implements ActionListener
{
 ...
 public SearchGuestBooking()
 {
 }
 //Protected Java Database Connection
 DatabaseAccess da = new DatabaseAccess();

 public void searchGuestBooking(String user)
 {
...
---------------------------
 ...
 menu11.addActionListener(e -> {
   ...
   mainBodySubPanel2.setBorder(BorderFactory.createLineBorder(Color.blue));
    ...
    JLabel lsearchNotice = new JLabel("Please enter or select one or
   more criteria to search by");
...
----------------------------
 ...
 try
 {
 dateBookedMask = new MaskFormatter("####-##-##"); //the # is for
numeric values
...
//Query to retrieve the Guest data from the database and populate
the Vector object
try
{
    String guestIdQuery = "SELECT guestId, Name FROM GUEST ORDER BY
    Name";
     PreparedStatement statement =
    da.con.prepareStatement(guestIdQuery);
     tguestIdModel.addElement(new Item("-1", "--Select a Guest
    Number--" ));
    ...
    ----------------------------
     ...
     //------------------------------------------------------------------
    ----------------------------
     //Search Booking Sub Panel 2: The Bill To Guest No textbox on the
    form
     //Query to retrieve the Guest data from the database and populate
    the Vector object
     try
     {
     String billToGuestIdQuery = "SELECT guestId, Name FROM GUEST
    ORDER BY Name";
     PreparedStatement statement =
    da.con.prepareStatement(billToGuestIdQuery);
     tbillToGuestIdModel.addElement(new Item("-1", "--Select a Guest
    Number--" ));
...
(Item)billToGuestIdComboBox.getSelectedItem();
 ...
 if(!s1.equals("") )
 {
 ViewGuestBooking vgb = new ViewGuestBooking();
 String[] vgbinfo = new String[6];
 vgbinfo[0] = s1;
 vgbinfo[1] = passedInfo[1];
 vgbinfo[2] = s2;
 vgbinfo[3] = s3;
 vgbinfo[4] = s4;
 vgbinfo[5] = s5;
 vgb.viewGuestBooking(vgbinfo);
 setVisible(true);
 this.dispose();
 }
 }
