...
public class StaffCreateBooking extends JFrame implements ActionListener {
    ...
    public void StaffCreateBooking(String user) {
        ...
        try {
            ...
            PreparedStatement statement =
                da.con.prepareStatement(roomTypeQuery);
        ...
        //Query to retrieve the Room data from the database and populate the Vector object
        try {
            String roomNoQuery = "SELECT r.roomNo, roomType FROM ROOM r WHERE r.roomNo NOT IN " + "(SELECT b.roomNo FROM BOOKING b WHERE b.checkInDate <= (SELECT CURRENT_DATE) " + "AND b.checkOutDate >= (SELECT CURRENT_DATE)) ORDER BY roomType";

            PreparedStatement statement =
                da.con.prepareStatement(roomNoQuery);
        ...
        Combobox with the data from the Vector
        ...
        try {
            ...
            PreparedStatement statement =
                da.con.prepareStatement(guestIdQuery);
        ...
        try {
            ...
            PreparedStatement statement =
                da.con.prepareStatement(billToGuestIdQuery);
        ...
        thotelStaffId.addActionListener(
            new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    JComboBox hotelsatffidComboBox =
                        (JComboBox) e.getSource();
                    Item item =
                        (Item) hotelsatffidComboBox.getSelectedItem();
                }
            });
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == submit) {
            try {
                String s1 = "";
                String s2 = tguestId.getSelectedItem().toString().substring(0, tguestId.getSelectedItem()
                    .toString().indexOf(' ')); // slected guestId
                String s3 = thotelStaffId.getSelectedItem().toString().substring(0,
                    thotelStaffId.getSelectedItem().toString().indexOf(' ')); // slected hotelStaffId
                String s4 = troomNo.getSelectedItem().toString().substring(0,
                    troomNo.getSelectedItem().toString().indexOf(' ')); // slected room no
                String s5 = tbillToGuestId.getSelectedItem().toString().substring(0,
                    tbillToGuestId.getSelectedItem().toString().indexOf(' ')); // slected billToGuestId
                String s6 = tcheckInDate.getText();
                // entered checkin date
                String s7 = tcheckOutDate.getText();
                // entered checkout date
                String s8 = tbookingStatus.getSelectedItem().toString();
                // selected booking status
                String s9 = troomType.getSelectedItem().toString();
                // slected room typ
                String s10 = "";
                if (tpaid.isSelected())
                    s10 = "1"; // checked if paid
                else
                    s10 = "0";

                String s11 = tdateBooked.getText();
                // entered date booked
                ...
                if (!s2.equals("") && !s3.equals("") && !s4.equals("") && !s5.equals("") && !s6.equals("")) {
                    ...
                    if (success > 0) {
                        query = "SELECT MAX(bookingId) FROM BOOKING WHERE guestId = ? AND hotelStaffId = ? AND roomNo = ? AND billToGuestId = ? AND checkInDate = ? AND checkOutDate = ? AND bookingstatus = ? AND roomType = ? AND paid = ? AND dateBooked = ?";
